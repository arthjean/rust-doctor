...
Against a second corpus of eight agent-authored repositories, selected by commit-trailer evidence alone with at least half of all commits carrying an agent attribution marker, agent-written Rust measures 5.4 structural findings per thousand lines against healthy Rust's 3.5, a ratio of 1.51.

That second population is now adjudicated too, and it is where the rules that had never fired on curated code turn out to work: `orphan_module_file` sits at 6.25% false positives over its whole population of 16 sites, `missing_lockfile` at 0% over 3. `complex_function`, the only rule measurable on both, moves from 87% on healthy code to 70% on agent code: the two rates differ, and they do not invert. Clippy is switched off on that population, since scanning untrusted code with the compiler would run its build scripts, so no Clippy rule can be measured there. The full record, confidence intervals included, sits in `docs/structural-precision-2026-08.md`, `docs/suppression-precision-2026-08.md`, `docs/top-rules-precision-2026-08.md` and `docs/agent-population-precision-2026-08.md`.
